...
from .utils import ONNXRuntimeError, read_yaml
from .utils import (TokenIDConverter, split_to_mini_sentence,code_mix_split_words,code_mix_split_words_jieba)
...
class CT_Transformer():
    ...
    def __init__(self, model_dir: Union[str, Path] = None,
        ...
        if backend != "cann":
            ...
            model_file = os.path.join(model_dir, 'punc.om')
            self.ort_infer = InferSession(device_id=0, model_path=model_file, debug=False)
    ...
    def __call__(self, text: Union[list, str], split_size=40):
        ...
        for mini_sentence_i in range(len(mini_sentences)):
            ...
            try:
                ...
                #        max_dex.append(2)
                #    else:
                #        max_dex.append(np.argmax(y[0,i]))
                #punctuations = np.array(max_dex)
            ...
            if mini_sentence_i < len(mini_sentences) - 1:
                ...
                if (sentenceEnd < 0 and len(mini_sentence) >= cache_pop_trigger_limit) or (len(mini_sentence) - sentenceEnd >= cache_pop_trigger_limit):# and last_comma_index >= 0:
                    # The sentence it too long, cut off at a comma.
                    if last_comma_index < 0:
                        # 句号逗号都没有，强制截断
                        #sentenceEnd = last_comma_index
                        punctuations[-1] = self.period
                        cache_sent = []
    ...
    def infer(self, feats: np.ndarray,
        ...
        if self.backend != "cann":
            outputs = self.ort_infer([feats, feats_len])
        else:
            outputs = self.ort_infer.infer([feats, feats_len], "dymshape", custom_sizes=1000000000)
            
        return outputs
        
    def free_source(self):
        if self.backend == "cann":
            self.ort_infer.free_resource()
