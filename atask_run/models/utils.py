...
import yaml
try:
    from onnxruntime import (ExecutionMode, GraphOptimizationLevel, InferenceSession,
                             SessionOptions)
except:
...
class OrtInferSession():
    def __init__(self, model_file, device_id=-1, intra_op_num_threads=2, backend="cpu"):

        sess_opt = SessionOptions()
        if intra_op_num_threads > 0:
            sess_opt.intra_op_num_threads = intra_op_num_threads
            # sess_opt.inter_op_num_threads = intra_op_num_threads    # 算子间单线程（顺序执行）

        # sess_opt.log_severity_level = 4
        # sess_opt.enable_mem_pattern = False      # ✅ 重要：禁用内存模式，大幅减少峰值内存
        ...
        if backend == "cann":
            providers = [
                    ...
                    "device id":0,
                    "enable cann graph": True,
                    "precision mode": "force_fp32",
                    })
            ]
