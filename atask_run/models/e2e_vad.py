...
class E2EVadModel:
    ...
    def __init__(self, vad_post_args: dict):
    ...
    def ComputeDecibel(self) -> None:
        ...
        for offset in range(0, self.waveform.shape[1] - frame_sample_length + 1, frame_shift_length):
            self.decibel.append(
                10
                * math.log10(np.square((self.waveform[0][offset : offset + frame_sample_length])).sum() + 0.000001))
        ...
        if AudioChangeState.kChangeStateSil2Speech == state_change:
            ...
            if self.vad_state_machine == VadStateMachine.kVadInStateStartPointNotDetected:
                ...
                if (
                    ...
                    if self.continous_silence_frame_count <= int(self.vad_opts.lookahead_time_end_point / frm_shift_in_ms):
                        self.OnVoiceDetected(cur_frm_idx)
                else:
                    self.MaybeOnVoiceEndIfLastFrame(is_final_frame, cur_frm_idx)
            else:
